...
const deletePostAndHierarchy = async (req, res) => {
  ...
  try {
    ...
    if (!isProdEnv) {
      ...
      try {
        const post = await Post.findOne({ _id: id, author: userId }).session(session);
        if (!post) {
        ...
        await Promise.all([
          Post.deleteMany({ _id: { $in: replies.map(r => r._id) } }).session(session),
          Post.deleteOne({ _id: id }).session(session),
          Media.deleteMany({ _id: { $in: mediaList.map(m => m._id) } }).session(session),
          User.updateMany(
            { _id: { $in: [post.author, ...replies.map(r => r.author)] } },
            { $inc: { posts_count: -1 } },
            { session }
