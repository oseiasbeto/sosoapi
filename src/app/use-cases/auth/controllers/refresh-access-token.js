...
const refreshAccessToken = async (req, res) => {
    try {
        ...
        if (!decodedData) return res.status(401).send({
        ...
        const expiresRefreshToken = process.env.JWT_REFRESH_TOKEN_EXPIRES_IN || '1y';
        ...
        user.activity_status.is_active = true
        user.activity_status.active_since = moment()
        user.activity_status.inactive_since = null

        // Responde com o novo token de acesso e os dados do usuário transformados
        res.status(200).send({
            access_token: accessToken,
            session_id: session.id,
            user: userTransformer(user), // Formata os dados do usuário antes de enviar
